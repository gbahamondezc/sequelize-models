...
console.log('belongsTo', model.getTableName(), attribute);

          model.belongsTo(refModel, {
            foreignKey : { name : attribute }
          });

console.log('hasMany', refModel.getTableName());
          refModel.hasMany(
            model
          );
      ...
      for (let tableName in belongsToMany) {
        ...
        lookupTable.forEach(primary => {
          if (indexes.hasOwnProperty(primary.fk)) {
            lookupTable.forEach(foreign => {
              if (primary !== foreign) {
                let attr = primary.model.attributes[ foreign.fk ];
                let indexComment = attr.comment;
                let fkComment = indexes[ primary.fk ];
                try {
                  indexComment = JSON.parse(indexComment);
                } catch(err) { 
                  indexComment = {as: indexComment};
                }
                try {
                ...
                let names = [indexComment.as, fkComment.as, foreign.refModel.name].filter(e => e);
                let name = null;
                let i = 1;
                while (true) {
                  try {
                    let as = names.length ? name = names.shift() : (name + i++);
console.log('belongsToMany', as, primary.model.name, primary.refModel.name, foreign.refModel.name);
                    primary.refModel.belongsToMany(foreign.refModel, {
                      through: primary.model,
                      as
                    });
                    break;
                  } catch(err) {
