...
class SequelizeModels {
  ...
  getModelDefinitions() {
    ...
    let modelsDirectory = path.join(path.resolve(), this.opts.models.path);
  ...
  getQueryInterface(sequelizeInstance) {
    return sequelizeInstance.getQueryInterface();
  }

  getTablesDescription(queryInterface, tables) {
    return Promise.all(tables.map(table => {
      return queryInterface.describeTable(table);
    }));
  }

  getTablesFromQueryInterface(queryInterface) {
    return queryInterface.showAllTables();
  }


  async getSchema() {
    let sequelize = this.getSequelizeInstance();
    let queryInterface = this.getQueryInterface(sequelize);
    let tables = await this.getTablesFromQueryInterface(queryInterface);
    let tablesDescription = this.getTablesDescription(queryInterface, tables);
