...
class PSQLDialect {
  ...
  constructor(Sequelize, connection, opts) {
    ...
    this.tablesQuery = `SELECT table_name AS name, column_name, data_type,
      column_default as column_key
    ...
    tableNames.map(name => {
      ...
      results.map(res => {
        ...
        attribs[res.column_name] = {
          ...
          primaryKey: (res.column_key && -1 !== res.column_key.indexOf('nextval')),
